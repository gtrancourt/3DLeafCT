...
def localthick_load_and_resize(folder_name, threshold_rescale_factor):
    ...
    if threshold_rescale_factor > 1:
        print('***RESIZING LOCAL THICKNESS STACK TO ORIGINAL SHAPE OF STACKS***')
        localthick_stack = transform.resize(localthick_small, [localthick_small.shape[0]*threshold_rescale_factor,localthick_small.shape[1],localthick_small.shape[2]], preserve_range=True)
    else:
...
def Threshold_GridPhase_invert_down(grid_img, phase_img, Th_grid, Th_phase,folder_name,rescale_factor):
    ...
    if rescale_factor == 1:
        ...
        tmp_invert_ds = transform.resize(tmp_invert, [tmp_invert.shape[0]/rescale_factor, tmp_invert.shape[1], tmp_invert.shape[2]])
        print("***SAVING IMAGE STACK***")
...
def Load_Resize_and_Save_Stack(filepath, stack_name, rescale_factor, keep_in_memory=True):
    if os.path.isfile(filepath + stack_name + "_" + str(rescale_factor) +"x-smaller.tif"):
        ...
        for idx in np.arange(stack_rs.shape[0]):
            stack_rs[idx] = transform.resize(stack[idx], [stack.shape[1]/rescale_factor, stack.shape[2]/rescale_factor], preserve_range=True)
        print("***SAVING RESIZED STACK " + stack_name + "***")
        io.imsave(filepath + stack_name + "_" + str(rescale_factor) +"x-smaller.tif", img_as_ubyte(stack_rs/255), imagej=True)
        if keep_in_memory == True:
            return stack_rs
...
def GTR_ML_wrapper(filepath,grid_name,phase_name,label_name,rescale_factor):
    ...
    # Trim the images so that they can be resized
    ...
    # Resize the local thickness stack if it has been resized
    if rescale_factor > 1:
    ...
    # That should do anything now that the stacks have been trimmed
