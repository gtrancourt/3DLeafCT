...
if os.path.isfile(folder_name+'/'+sample_name+'GridPhase_invert_ds.tif') == False:
    ...
    Threshold_GridPhase_invert_down(gridrec_stack,phaserec_stack,Th_grid,Th_phase,folder_name,sample_name,threshold_rescale_factor)
...
if os.path.isfile(folder_name+sample_name+'RF_model.joblib'):
    ...
    joblib.dump(rf_transverse, folder_name+sample_name+'RF_model.joblib', compress='zlib')#print_feature_layers(rf_transverse,folder_name)
...
io.imsave(folder_name+sample_name+"fullstack_prediction.tif", img_as_ubyte(RFPredictCTStack_out/RFPredictCTStack_out.max()))
print('Done!')
